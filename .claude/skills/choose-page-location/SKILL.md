...
Decide where a page belongs in the `wiki/pages/` folder hierarchy and, if needed, create a new subdirectory.
...
3. Create a subdirectory only when:
   ...
   - Immediately invoke `create-index` so the new directory gets an `_index.md`.
4. Confirm the target path with the caller before writing any files.
5. Refresh the parent directory index after the page is placed by invoking `create-index` on the parent directory.

## Rules

- Never create a directory for a single page unless it is a clear entry point for a future cluster.
